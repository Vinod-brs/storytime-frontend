...
import { useDeleteCategoryAPIMutation, useUpdateCategoryAPIMutation } from "../../../store/admin/adminApiSlice";
import { toast, ToastContainer } from "react-toastify";
import AddCategory from "./AddCategory";

const CategoriesTableWithSearch = ({ data, onUpdate, onDelete }) => {
  ...
  const [updateCategoryAPI, {isLoading}] = useUpdateCategoryAPIMutation();
    const [deleteCategoryAPI, {isLoading: deleteLoading}] = useDeleteCategoryAPIMutation();
    ...
    const handleSubmit = async (rowIndex) => {
      ...
      try {
        // console.log(row._id)
        const response = await updateCategoryAPI({id: row._id, category: row.category, keywords: row.keywords   })
    ...
    const handleDelete = async (rowIndex) => {
      ...
      if (!isConfirmed) {
        toast.success('Category restoration successful – deletion undone.')
        return; 
      }
    
      try {
        const response = await deleteCategoryAPI({ id: row._id });
  ...
  return (
    ...
    <div className="p-4">
      ...
      <h1 className="text-xl font-bold mb-4">Search Categories</h1>
...
export default CategoriesTableWithSearch;